...
if (nameArg) {
    if (allowedPackages.includes(nameArg.split('=')[1])) {
        ...
        rl.question(`Are you sure you want to migrate up on package '${nameArg.split('=')[1]}'? yes/no\n`, (answer) => {
            if (answer === 'yes') {
                migrateToLatest(nameArg.split('=')[1]).then((_) => {
        ...
        console.error(`Please provide a valid package, one of: ${allowedPackages.join(', ')}`);
    }
