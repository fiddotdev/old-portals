...
export const up = async (db: Kysely<any>) => {
    ...
    await db.schema
        ...
        .addColumn('mentionsPositions', sql`smallint[]`, (col) =>
            col.notNull().defaultTo(sql`'{}'`)
        )
        .addUniqueConstraint('casts_hash_unique', ['hash'])
        .addForeignKeyConstraint('casts_hash_foreign', ['hash'], 'messages', [
            'hash',
        ])
        .execute();
    ...
    await db.schema
        ...
        .addForeignKeyConstraint('reactions_hash_foreign', ['hash'], 'messages', [
            'hash',
        ])
        .execute();
    ...
    await db.schema
        ...
        .addForeignKeyConstraint('signers_hash_foreign', ['hash'], 'messages', [
            'hash',
        ])
        .execute();

    await db.schema
        .createIndex('signers_fid_timestamp_index')
        .on('signers')
        .columns(['fid', 'timestamp'])
        .execute();
    ...
    await db.schema
        ...
        .addForeignKeyConstraint('user_data_hash_foreign', ['hash'], 'messages', [
            'hash',
        ])
        .execute();

    await db.schema
        .createIndex('user_data_fid_index')
        .on('userData')
        .columns(['fid'])
        .execute();
