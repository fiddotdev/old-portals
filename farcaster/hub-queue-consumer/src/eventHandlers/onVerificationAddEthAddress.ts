...
const onVerificationAddEthAddress = async (
  ...
  await db
    ...
    .values(
      messages.map((message) => {
        ...
        const address = safeUnwrapForHubEvent(() =>
          bytesToHexString(
            message.data.verificationAddEthAddressBody.address
          )._unsafeUnwrap()
        );
        const ethSignature = safeUnwrapForHubEvent(() =>
          bytesToHexString(
            message.data.verificationAddEthAddressBody.ethSignature
          )._unsafeUnwrap()
        );
        const blockHash = safeUnwrapForHubEvent(() =>
          bytesToHexString(
