...
const onLinkAdd = async (messages: LinkAddMessage[], isInitialCreation: Record<string, boolean>) => {
    await db
        .insertInto('links')
        .values(
            messages.map((message) => ({
                ...
                displayTimestamp: message.data.linkBody.displayTimestamp ? farcasterTimeToDate(message.data.linkBody.displayTimestamp) : undefined,
            }))
        )
        .execute();
